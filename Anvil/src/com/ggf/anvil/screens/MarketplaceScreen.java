					...
					public void clicked(InputEvent event, float x, float y) {
						if (isItemSelected) {
							if(originatedPlayer) {
								...
								//TODO: TH: show price when picking up, change to selling action later
//								Label myPrice = new Label("+"+selectedItem.price+" $", new Label.LabelStyle(font, Color.WHITE));
...
//								fg.addActor(myPrice);
					...
					public void clicked(InputEvent event, float x, float y) {
						if (isItemSelected) {
							if(!originatedPlayer) {
								...
								//TODO: TH: show price when picking up, change to selling action later
								//Label myPrice2 = new Label("+"+selectedItem.price+" $", new Label.LabelStyle(font, Color.WHITE));
		...
		playerName = new Label(ProfileManager.getInstance().retrieveProfile().getPlayer().name+":", 
		...
		String greetMessage = greetings[new Random().nextInt(greetings.length)]
				             .replace("$MNAME$", currentMerchant.name)
				             .replace("$NAME$", ProfileManager.getInstance().retrieveProfile().getPlayer().name);

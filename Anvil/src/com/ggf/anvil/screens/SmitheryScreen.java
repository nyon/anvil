...
public class SmitheryScreen extends AbstractScreen {
	...
	public void show() {
        ...
        Texture eastereggtext = new Texture(new Pixmap(new Gdx2DPixmap(1,1,Gdx2DPixmap.GDX2D_FORMAT_RGBA8888)));
        Image easteregg = new Image(eastereggtext);
        easteregg.setPosition(5, 5);
        easteregg.addListener(new InputListener(){
			@Override
			public boolean touchDown(InputEvent event, float x, float y,
					int pointer, int button) {

				SoundManager.getInstance().play(AnvilSound.FART);
				return true;
			}
		}
		);
        fg.addActor(easteregg);
		...
		final BitmapFont font = new BitmapFont(Gdx.files.internal("skin/default.fnt"), false);

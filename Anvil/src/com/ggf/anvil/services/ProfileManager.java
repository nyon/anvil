...
public class ProfileManager
{
  ...
  public Profile retrieveProfile()
  {
    ...
    Gdx.app.log(Anvil.LOG, "Retrieving profile from: " + profileDataFile.path());
    ...
    if (profileDataFile.exists())
    {
      ...
      try
      {
        ...
        {
          Gdx.app.log(Anvil.LOG, "Persisted profile is base64 encoded");
      ...
      catch (Exception e)
      {
        ...
        // log the exception
        Gdx.app.error(Anvil.LOG, "Unable to parse existing profile data file", e);
  ...
  protected void persist(Profile profile)
  {
    // create the handle for the profile data file
    FileHandle profileDataFile = Gdx.files.local(PROFILE_DATA_FILE);
    Gdx.app.log(Anvil.LOG, "Persisting profile in: " + profileDataFile.path());
    ...
    // encode the text
    /*
     * if(!Anvil.DEV_MODE){ profileAsText = Base64Coder.encodeString(
     * profileAsText ); }
     */
